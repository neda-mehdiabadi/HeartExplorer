  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "about",
              ...
              h3("Authors:"),
              div(p("Neda R. Mehdiabadi, Choon Boon Sim, Belinda Phipson, Ravi K. R. Kalathur, Yuliangzi Sun, Celine J. Vivien, Menno ter Huurne, Adam T. Piers, James E. Hudson, Alicia Oshlack, Robert G. Weintraub, Igor E. Konstantinov, Nathan J. Palpant, David E. Elliott, Enzo R. Porrello"),style="text-align: justify;"),
              h3("Abstract"),
              h3("Background:"),
      ...
      tabItem(tabName = "visualization",
              ...
              plotOutput("feature"),
              br(),
              column(6,align = "center", offset = 3, selectInput("selectcluster", label = "Select a cell type:", choices = c("Cardiomyocytes"="CM","Endothelial cells" = "Endo","Pericyte"="Pericyte","Fibroblast"="Fib","Immune cells"="Immune","Neurons"="Neuron","Smooth muscle cells"="Smc"), selected = "Cardiomyocytes", multiple=FALSE, selectize=TRUE)),
              fluidRow(
      ...
      tabItem(tabName = "enrichment", useShinyjs(),
              ...
              fluidRow(
                column(5,offset=1, align = "center",radioButtons("group1", "Select group 1:", 
                                      choices = c("Fetal","ND","DCM"), inline = TRUE, selected = "Fetal")),
                column(6,align = "center",radioButtons("group2", "Select group 2:", 
                                        choices = c("Fetal","ND","DCM"), inline = TRUE, selected = "ND")),
                      ),
              column(6,align = "center", offset = 3, selectInput("selectclusterforenrichment", label = "Select a cell type:", choices = c("Cardiomyocytes"="CM","Endothelial cells" = "Endo","Pericyte"="Pericyte","Fibroblast"="Fib","Immune cells"="Immune","Neurons"="Neuron","Smooth muscle cells"="Smc"), selected = "Cardiomyocytes", multiple=FALSE, selectize=TRUE)),
              fluidRow(
      ...
      tabItem(tabName = "code",
              ...
              HTML(c("<p>Also, the code used to build this website is available on <a href='https://github.com/neda-mehdiabadi/HeartExplorer' target = '_blank'><b> github/HeartExplorer</b></a>.")),
      )
    )
  )
)
...
server <- function(input,output,session) {
  ...
  output$umap <- renderPlot({
    DimPlot(cds, reduction = "umap",label=FALSE,label.size = 6, cols = c("Er" = "brown4", "CM(Prlf)"= "darkorange2","CM"="#7570B3","Endo"="#E7298A","Pericyte"="#66A61E","Fib"="#E6AB02","Immune"="#A6761D","Neuron"="#666666","Smc"="#1F78B4"), split.by = "orig.ident")
  })
  ...
  output$vlnclust <- renderPlot({
    ...
    p <- VlnPlot(cds, features = gene,pt.size = 0.1,  split.by = "orig.ident",idents = paste(levels(cds$orig.ident),cluster,sep = "."),y.max = max+2, split.plot = F, cols = c("#FBC6E7","#9A6FD6","#D1FF66"))+xlab("")+NoLegend()
  ...
  output$vlnclustbybiorep <- renderPlot({
    ...
    chr <- tolower(substr(cluster,1,2))
    max = max(cds@assays$SCT@data[gene,])
    p <- VlnPlot(cds, features = gene,pt.size = 0.1, idents = paste(levels(cds$biorep),cluster,sep = "."), y.max = max+2, split.plot = F, cols = c("#FBC6E7","#EF6EB8","#C0297B","#4D2082","#7E51B7","#9A6FD6","#D1FF66","#9DF25C","#3EC75E","#3E8C54"))+xlab("")+NoLegend()
  ...
  index <- which(ds$gene==input$selectgene)
   if (length(index) != 0){
       HTML(c(paste(ds$gene[index]," is differentially expressed in ",ds$cluster[index], "versus all other cells with adjusted p-value of", signif(ds$p_val_adj[index],digits = 3),'<br/>')))
    } else {
      HTML(c("The gene is not detected as a differentially expressed gene in any cell types by comparing each cell type versus all other cells."))
    }
  })
